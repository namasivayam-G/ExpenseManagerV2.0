...
public class DashboardService implements IDashboardService{
	...
	public Map<String,List<ExpenseItem>> getMonthlyExpensesByCategory(long userId, String year, String month,
		...
		List<ExpenseItem> expenseList =new ArrayList<>();
			...
			JsonNode  nodeList = restTemplate.getForObject(baseUrl+"expense/monthly-expense/"+userId+"/"+year+"/"+month, JsonNode.class);
			ArrayList<ExpenseItem> expenseItemList = mapper.convertValue(
					nodeList, new TypeReference<ArrayList<ExpenseItem>>(){});
			 /*expenseItemList.stream().map(expenseItem-> {
			...
			}).collect(Collectors.toMap(ExpenseItem::getItemCategory,expenseList));*/
			for (ExpenseItem expenseItem : expenseItemList) {
				if(category.equalsIgnoreCase(expenseItem.getItemCategory())){
					expenseList.add(expenseItem);
					expenseCategoryMAp.put(category, expenseList);
				}else{
					Response response =new Response("No category found");
					throw new DashboardException(response);
				}
			}
		return expenseCategoryMAp;
	}
	...
	public Map<String, List<ExpenseItem>> getweeklyExpensesByCategory(long userId, String year, String month,
		...
		try{
			...
			String baseUrl=serviceInstance.getUri().toString();
			JsonNode  nodeList = restTemplate.getForObject(baseUrl+"expense/weekly-expense/"+userId+"/"+year+"/"+month+"/"+fromDate+"/"+toDate, JsonNode.class);
			ArrayList<ExpenseItem> expenseItemList = mapper.convertValue(
					nodeList, 
	...
	public Map<String, List<ExpenseItem>> getDailyExpensesByCategory(long userId, String year, String month, String day,
		...
		try{
			...
			String baseUrl=serviceInstance.getUri().toString();
			JsonNode  nodeList = restTemplate.getForObject(baseUrl+"expense/daily-expense/"+userId+"/"+year+"/"+month+"/"+day, JsonNode.class);
			ArrayList<ExpenseItem> expenseItemList = mapper.convertValue(
					nodeList, 
				    new TypeReference<ArrayList<ExpenseItem>>(){}
				);
